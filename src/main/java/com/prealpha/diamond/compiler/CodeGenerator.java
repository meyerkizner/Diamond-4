...
final class CodeGenerator extends ScopeAwareWalker {
     ...
     * effect, the phase may generate multiple errors if needed, instead of stopping after the first error in the phase.
     */
    ...
    /**
     * <p>
     *     Constructs a new {@code CodeGenerator} using the given inputs from previous phases. {@code scopeSource} is
     *     used to invoke {@link super#ScopeAwareWalker(ScopeAwareWalker)}, which initializes the scope information for
     *     the syntax tree we are walking through. {@code exceptionBuffer} is used to return exceptions to the invoking
     *     context. {@code types} is the type information in the format emitted by
     *     {@link com.prealpha.diamond.compiler.TypeEnforcer#getTypes()}.
     * </p>
     *
     * <p>
     *     The code generator should be used by invoking {@link Node#apply(com.prealpha.diamond.compiler.node.Switch)}
     *     on the root of the syntax tree. The syntax tree <i>must</i> be the same tree which was used to generate
     *     {@code scopeSource} and {@code types}, otherwise the result of the code generation is undefined.
     * </p>
     *
     * @param scopeSource the scope information for this syntax tree
     * @param exceptionBuffer a list of exceptions, which is modified as exceptions occur during code generation; the
     *                        {@code CodeGenerator} retains a reference to the same list as is passed in, rather than
     *                        making a copy
     * @param types the type information for this syntax tree, as generated by {@code TypeEnforcer}
     */
